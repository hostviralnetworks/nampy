...
available_mapping = {"UniProtKB AC/ID":"ACC+ID", 
               "UniProtKB": "ACC",
               "UniProtKB": "ID",
               "UniParc": "UPARC",
               "UniRef50": "NF50",
...
default_mapping_list = ["UniProtKB AC/ID", 
    "UniProtKB",
    "EMBL/GenBank/DDBJ",
...
def get_more_node_ids(the_network, **kwargs):
    ...
    if 'mapping_types' in kwargs:
        ...
        mapping_types = default_mapping_list

    mapping_dict = {}
    for the_type in mapping_types:
        mapping_dict[the_type] = available_mapping[the_type]
...
def get_more_source_dict_ids(source_dict, **kwargs):
    ...
    if continue_flag:
        if file_key != 'default':
            ...
            for the_type in mapping_types:
                mapping_dict[the_type] = available_mapping[the_type]
            mapping_dict[file_key] = available_mapping[file_key]
            if len(mapping_types) > 0:
                ...
                for the_target_type in mapping_types:
                    the_result = u.mapping(fr = mapping_dict[file_key], to = mapping_dict[the_target_type], query = query_string)
